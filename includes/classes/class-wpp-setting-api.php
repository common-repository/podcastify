...
if ( ! class_exists( 'Wpp_Setting_API' ) ) :

	class Wpp_Setting_API {
		...
		function callback_text( $args ) {
			...
			$html  = sprintf( '<input type="%1$s" class="%2$s-text %7$s wppfy-text" id="%3$s[%4$s]" name="%3$s[%4$s]" value="%5$s"placeholder="%6$s"/>', $type, $size, $args['section'], $args['id'], $value, $args['placeholder'], $args['class'] );
		...
		function callback_checkbox( $args ) {
			...
			$html  = '<fieldset>';
			$html .= sprintf( '<label class="wppfy-label checkbox-label" for="wppfy-%1$s[%2$s]">', $args['section'], $args['id'] );
			$html .= sprintf( '<input type="hidden" name="%1$s[%2$s]" value="off" />', $args['section'], $args['id'] );
			$html .= sprintf( '<input type="checkbox" class="checkbox wppfy-checkbox %4$s" id="wppfy-%1$s[%2$s]" name="%1$s[%2$s]" value="on" %3$s /><span class="check-slider"></span>', $args['section'], $args['id'], checked( $value, 'on', false ), $args['class'] );
		...
		function callback_multicheck( $args ) {
			...
			foreach ( $args['options'] as $key => $label ) {
				...
				$html   .= sprintf( '<label class="wppfy-label checkbox-label" for="wppfy-%1$s[%2$s][%3$s]">', $args['section'], $args['id'], $key );
				$html   .= sprintf( '<input type="checkbox" class="checkbox wppfy-checkbox wppfy-multi  %5$s" id="wppfy-%1$s[%2$s][%3$s]" name="%1$s[%2$s][%3$s]" value="%3$s" %4$s /><span class="check-slider"></span>', $args['section'], $args['id'], $key, checked( $checked, $key, false ), $args['class'] );
		...
		function callback_radio( $args ) {
			...
			$html = '<fieldset>';
			foreach ( $args['options'] as $key => $label ) {
				$html .= sprintf( '<label class="wppfy-label" for="wppfy-%1$s[%2$s][%3$s]">', $args['section'], $args['id'], $key );
				$html .= sprintf( '<input type="radio" class="radio wppfy-radio %5$s " id="wppfy-%1$s[%2$s][%3$s]" name="%1$s[%2$s]" value="%3$s" %4$s />', $args['section'], $args['id'], $key, checked( $value, $key, false ), $args['class'] );
		...
		function callback_select( $args ) {
			...
			$html = sprintf( '<select class="%1$s %4$s wppfy-select" name="%2$s[%3$s]" id="%2$s[%3$s]">', $size, $args['section'], $args['id'], $args['class'] );
			foreach ( $args['options'] as $key => $label ) {
		...
		function callback_multiselect( $args ) {
			...
			$html   = sprintf( '<select class="%1$s %4$s wppfy-select wppfy-multi" name="%2$s[%3$s][]" id="%2$s[%3$s]" multiple>', $size, $args['section'], $args['id'], $args['class'] );
			$_value = array_flip( $value );
			foreach ( $args['options'] as $key => $label ) {
				$selected = isset( $_value[ $key ] ) ? esc_attr( $key ) : '';
				$html    .= sprintf( '<option value="%1s" %2s>%3s</option>', $key, selected( $key, $selected, false ), $label );
			}
			$html .= sprintf( '</select>' );
			$html .= $this->get_field_description( $args );
		...
		function callback_textarea( $args ) {
			...
			$html  = sprintf( '<textarea rows="5" cols="55" class="%1$s-text %5$s wppfy-textarea" id="%2$s[%3$s]" name="%2$s[%3$s]">%4$s</textarea>', $size, $args['section'], $args['id'], $value, $args['class'] );
			$html .= $this->get_field_description( $args );
		...
		function callback_file( $args ) {
			...
			$html  = sprintf( '<input type="text" class="%1$s-text wpffy-file wpsa-url" id="%2$s[%3$s]" name="%2$s[%3$s]" value="%4$s"/>', $size, $args['section'], $args['id'], $value );
		...
		function callback_image( $args ) {
			...
			$html  = sprintf( '<input type="text" class="%1$s-text wpsa-url wppfy-text %5$s" id="%2$s[%3$s]" name="%2$s[%3$s]" value="%4$s"/>', $size, $args['section'], $args['id'], $value, $args['class'] );
		...
		function callback_password( $args ) {
			...
			$html  = sprintf( '<input type="password" class="%1$s-text wppfy-text wppfy-password" id="%2$s[%3$s]" name="%2$s[%3$s]" value="%4$s"/>', $size, $args['section'], $args['id'], $value );
			$html .= $this->get_field_description( $args );
		...
		public function plugin_page() {
			echo '<div class="wrap wppfy-setting-wrapper">';
			echo '<h1>Podcastify <span style="font-size:50%;"></span></h1>
							<div class="wppfy-setting-inner">';
		...
		function show_navigation() {
			$html = '<div class="wppfy-aside"><h2 class="nav-tab-wrapper">';
		...
		function script() {
			...
			<style type="text/css">
				...
				.series-tab-wrapper .nav-tab:focus{
					box-shadow:none;

				}
				.series-tab-wrapper:after{
					content: "";
					display: table;
					clear: both;
				}
				#wpp_feed .form-table tr:not(.show){
					display:none;
				}
			</style>
			<?php
